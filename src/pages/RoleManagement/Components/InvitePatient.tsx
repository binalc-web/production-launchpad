...
import {
  invitePatientApi,
  type InvitePatientType,
} from '@/api/roleManagement/invitePatient';
import { PopUp } from '@/components/Popup';
import ToastAlert from '@/components/ToastAlert';
import { trackEvent } from '@/utils/mixPanel/mixpanel';
import { IndividualEntityRole } from '@/pages/Register/steps/data';
...
const InvitePatient: React.FC<InvitePatientPopOverProps> = ({
  open,
  handleClose,
}) => {
  const {
  ...
  const {
    ...
    mutationFn: async (inviteData: InvitePatientType) => {
      ...
      const response = await invitePatientApi(inviteData);
      return response;
    },
  });
          ...
          >
            <Box width={'100%'}>
              ...
              >
                ...
                {IndividualEntityRole.map((IndividualEntry) => (
                  <MenuItem
                    key={IndividualEntry.type}
                    value={IndividualEntry.type}
                  >
                    {IndividualEntry.title}
                  </MenuItem>
                ))}
              </Select>
