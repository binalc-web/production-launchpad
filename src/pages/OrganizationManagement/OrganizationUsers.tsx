...
import { TrashIcon, UserCirclePlusIcon, XIcon } from '@phosphor-icons/react';
import { roles } from '../Register/steps/data';
import { changeActivationStatus } from '@/api/userManagement/changeActivationStatus';
import { getAllOrganizationUsers } from '@/api/OrganizationManagement/getAllOrganizationUsers';
import { useNavigate, useParams, useSearch } from '@tanstack/react-router';
import { deleteOrganization } from '@/api/OrganizationManagement/deleteOrganization';
import InviteUser from './OrganizationAdmin/Components/InviteUser';
import { getUserRoleByOrganizationType } from '@/utils/getUserRoleByOrganizationType';
...
const OrganizationUsers: React.FC = () => {
  const parameters = useParams({ from: '/_app/organization/users/$id' });

  const searchParameters: searchParametersType = useSearch({
    from: '/_app/organization/users/$id',
  });
  ...
  const [showInviteUserPopup, setShowInviteUserPopup] =
    useState<boolean>(false);
  ...
  //     refetch: () => void;
  //   } = useQuery({
  //     queryKey: ['organization', parameters.id],
  //     queryFn: async () => {
  //       const data = await getOrganizationDetails(parameters.id);
  //       return data;
  //     },
  //     staleTime: Infinity,
  ...
  const {
    ...
    queryKey: [`users-${parameters.id}`],
    queryFn: async () => {
      const data = await getAllOrganizationUsers(filters, parameters.id);
  ...
  const {
    ...
    isError: isErrorWhileDeleteOrganization,
    error: errorWhileDeleteOrganization,
    isSuccess: isSuccessWhileDeleteOrganization,
  } = useMutation({
    mutationFn: async ({ organizationId }: { organizationId: string }) => {
      // Call the API to delete  organization
      const response = await deleteOrganization(organizationId);
      return response;
    },
    onSuccess: () => {
      void trackEvent('Organization deleted successfully.');
      //refetch();
      //reset();
      //handleClose();
    },
  ...
  useEffect(() => {
    if (isSuccessWhileDeleteOrganization) {
    ...
    if (isErrorWhileDeleteOrganization) {
    ...
    isSuccessWhileDeleteOrganization,
    isErrorWhileDeleteOrganization,
    navigate,
  ]);
  ...
  }, [userList, isLoading, isError, parameters.id]);
   ...
   * @function
   * @description close the invite user popup by setting the state to false
   */
  const handleClose = (): void => {
  ...
  return (
    <>
      ...
      <InviteUser
        ...
        userRole={getUserRoleByOrganizationType(
          searchParameters.organizationType
        )}
      ...
      {showDeleteOrganizationPopup ? (
        <PopUp
          type="DELETETASK"
          cancelText="No, Not yet"
          title={
            <Typography fontWeight={600} fontSize={24} variant="h2">
              {`Are you sure you want to delete an organization and its users?`}
            </Typography>
          }
          buttonText={`Yes, Delete it`}
          isOpen={showDeleteOrganizationPopup}
          description=""
          onCancel={() => {
            setShowDeleteOrganizationPopup(false);
          }}
          onClick={() => {
            mutateDeleteOrganization({ organizationId: parameters.id });
            setShowDeleteOrganizationPopup(false);
          }}
        />
      ) : null}
      ...
      {showDeleteError ? (
        <ToastAlert
          ...
          message={
            errorWhileDeleteOrganization instanceof Error
              ? errorWhileDeleteOrganization.message
