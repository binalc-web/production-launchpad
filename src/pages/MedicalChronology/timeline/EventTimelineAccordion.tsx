...
const EventTimelineAccordion: FC<EventTimelineAccordionProps> = (props) => {
  ...
  // Track which accordion is expanded locally by event _id (unique per event)
    ...
    >
      ...
      <Box
        sx={{
          ...
          maxHeight: 'calc(100vh - 18rem)', // Slightly adjusted to account for the header height
          pb: 3,
        }}
      >
          ...
          <Accordion
            ...
            onChange={(_, isExpanded) => {
              if (isExpanded) {
                ...
                if (item?.file?._id !== parameters.id) {
                  void router.navigate({
                    to: `/medical-chronology/timeline-event/${parameters.chronologyId}/${item?.file?._id}`,
                    replace: true,
                  });
                  setSelectedFile({
                    id: item?.file?._id,
          ...
          >
            ...
            <AccordionDetails>
              ...
              {isEditMode ? (
                ...
                <Typography>{item?.translated_summary}</Typography>
              )}
              <Box
                sx={{
                  mt: 1.25,
