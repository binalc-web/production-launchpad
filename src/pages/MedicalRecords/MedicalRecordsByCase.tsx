...
// Define a type for the patient details object structure
...
const MedicalRecordsByCase: React.FC = () => {
  ...
  const { id } = useParams({
    from: '/_app/medical-records/case/$id',
  });
  ...
  const {
    ...
    queryKey: ['medicalRecordsByCase', filters],
    queryFn: async () => {
      ...
      return getMedicalRecordsByCase(mappedFilters, id);
    },
    staleTime: 0,
    ...
    if (recordList?.medicalRecords?.length) {
      recordList?.medicalRecords.forEach((element: MedicalRecordTableType) => {
        const caseItem = {
          _id: element?._id || '',
          caseId:
            typeof element?.caseId === 'string'
              ? element.caseId
              : element?.caseId?.caseId || '',
          patientId: element?._id?.toString() || '',
          ...
          patientDetails: {
            firstName:
              typeof element?.patientId === 'object'
                ? (element.patientId as PatientDetailsType)?.firstName || ''
                : '',
            lastName:
              typeof element?.patientId === 'object'
                ? (element.patientId as PatientDetailsType)?.lastName || ''
                : '',
            email:
              typeof element?.patientId === 'object'
                ? (element.patientId as PatientDetailsType)?.email || ''
                : '',
          },
...
export default MedicalRecordsByCase;
